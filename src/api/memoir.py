...
@router.post(
    ...
    response_model=MemoirResponseEnvelope
)
def create_memoir(
    ...
    This must be executed first to obtain a memoir_id before adding
    media or memories.
    """
    ...
    new_memoir = MemoirService.create_memoir(
        payload,
        user_session
    )
...
def get_my_memoir(
    ...
    """
    Fetches the memoir created by the currently authenticated user.
    """
    ...
    memoir = MemoirService.get_memoir_by_user_id(user_id)

    if not memoir:
        return {
            "success": True,
            "message": "No memoir found.",
            "data": None
        }

    return {
        "success": True,
        "message": "Memoir fetched successfully.",
        "data": memoir
    }